...
class TreeModelMD(nn.Module):
    def __init__(self, opt):
        super(TreeModelMD, self).__init__()
        self.opt = opt
        self.max_length = 40
    ...
    def from_scratch(self, feature):
        ...
        output, (hidden, cell) = self.md_lstm(word_embed, input_f, (h_a, c_a), (h_f, c_f))
    ...
    def beam_step(self, state_a, x_a, state_f=None, x_f=None):
        ...
        h_a, c_a = state_a
        output, (hidden, cell) = self.md_lstm(word_embed_a, word_embed_f, (h_a, c_a), (h_f, c_f))
        logits = self.logit(hidden)
        logprobs = F.log_softmax(logits, dim=1)
        return (hidden, cell), logprobs
