...
class TreeModelMDSOB(nn.Module):
    def __init__(self, opt):
        super(TreeModelMDSOB, self).__init__()
        self.opt = opt
        self.max_length = 40
    ...
    def forward(self, word_idx, father_idx, fc_feats):
        ...
        start_token = torch.zeros(batch_size, requires_grad=False).long().fill_(3).cuda()
        start_token = self.embed(start_token)
        ...
        for i in range(word_idx.size(1)):
            if i == 0:
                ...
                _, h_a = self.md_lstm(x_a, x_f, h_a, h_f)
                x_a = start_token
    ...
    def from_scratch(self, feature):
        start_token = torch.zeros(1).long().fill_(3)
        start_token = self.embed(start_token)
        ...
        hidden, cell = self.md_lstm(start_token, input_f, (hidden, cell), (h_f, c_f))
