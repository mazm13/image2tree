...
class TreeModel2(nn.Module):
    def __init__(self, opt):
        super(TreeModel2, self).__init__()
        self.opt = opt
        ...
        self.linear_h = nn.Linear(opt.hidden_size * 2, opt.hidden_size)
        self.linear_c = nn.Linear(opt.hidden_size * 2, opt.hidden_size)
    ...
    def _forward(self, node):
        ...
        hs = torch.tanh(self.linear_h(torch.cat([h_a, h_f], dim=1)))
        cs = torch.tanh(self.linear_c(torch.cat([c_a, c_f], dim=1)))
        node.h = hs, cs
